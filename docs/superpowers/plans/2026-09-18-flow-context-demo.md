...
- Keep real conversation and meeting data in ignored local JSON. Committed example data must be labeled synthetic and must not portray hypothetical meetings as real.
...
**Interface:** `load_moments(path: Path, thread_id: str) -> list[Moment]`. A `Moment` has `id`, `date`, `thread_id`, `source_type` (`voice`, `ai_chat`, `meeting`, `document`), `author_role` (`user`, `ai`, `other`), `stance` (`observation`, `proposal`, `rejection`, `decision`, `open_question`), and `text`. IDs are unique; moments are sorted by date. `config.py` defines `LlmConfig(base_url: str, model: str, api_key: str)` and an active thread title.

- [ ] Write `tests/test_store.py` with a valid two-moment fixture and assertions that malformed source types, duplicate IDs, and missing attribution fail validation.
- [ ] Run `python -m pytest tests/test_store.py -q`; expect the initial test to fail because the package does not exist.
- [ ] Add the package files, `pyproject.toml`, `.env.example`, and ignore rules. `config.py` calls `load_dotenv()`; it reads `FLOW_CONTEXT_MOMENTS_PATH`, `FLOW_CONTEXT_THREAD_ID`, `FLOW_CONTEXT_THREAD_TITLE`, `FLOW_CONTEXT_LLM_BASE_URL`, `FLOW_CONTEXT_LLM_MODEL`, and `FLOW_CONTEXT_LLM_API_KEY` from the environment after `.env` is loaded. The example file uses synthetic moments inspired by the shared thread: reflection idea, AI proposal, user's workplace objection, cross-app context direction, and unresolved intervention question.
...
Why it changed: ...
Still open: ...
Sources: voice-01, chat-02, chat-04
...
- [ ] Write `flow-context/README.md` with Python setup, `.env` creation from `.env.example`, and `python -m uvicorn flow_context.server:app --host 127.0.0.1 --port 8000`. The app itself loads `.env` with `python-dotenv`; commands contain no credential values.
- [ ] Install and launch the official Handy macOS release. Grant Microphone and Accessibility, download/select a transcription model, and verify ordinary dictation in a plain text field.
