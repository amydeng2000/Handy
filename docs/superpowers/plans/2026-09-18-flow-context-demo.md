...
- Treat the supplied file as one synthetic thread. The user names the problem in the demo utterance; the MVP has no topic classifier.
...
| `docs/flow-context-conversation-template.md` | Copyable user intake format for one synthetic thread |
...
| `flow-context/flow_context/store.py` | Read and validate the supplied thread's JSON moments |
...
**Interface:** `load_moments(path: Path) -> list[Moment]`. A `Moment` has `id`, `date`, `source_type` (`voice`, `ai_chat`, `meeting`, `document`), `author_role` (`user`, `ai`, `other`), `stance` (`observation`, `proposal`, `rejection`, `decision`, `open_question`), `origin` (`real`, `synthetic`), and `text`. IDs are unique; moments are sorted by date. `config.py` defines `LlmConfig(base_url: str, model: str, api_key: str)` and the thread title.

- [ ] Write `tests/test_store.py` with a valid two-moment fixture and assertions that malformed source types, duplicate IDs, missing attribution, and missing origin fail validation.
- [ ] Run `python -m pytest tests/test_store.py -q`; expect the initial test to fail because the package does not exist.
- [ ] Add the package files, `pyproject.toml`, `.env.example`, and ignore rules. `config.py` calls `load_dotenv()`; it reads `FLOW_CONTEXT_MOMENTS_PATH`, `FLOW_CONTEXT_THREAD_TITLE`, `FLOW_CONTEXT_LLM_BASE_URL`, `FLOW_CONTEXT_LLM_MODEL`, and `FLOW_CONTEXT_LLM_API_KEY` from the environment after `.env` is loaded. The store rereads JSON on each dictation so new records appear without a service restart. The example file uses synthetic moments inspired by the shared thread: reflection idea, AI proposal, user's workplace objection, cross-app context direction, and unresolved intervention question. Convert the user's filled conversation template into local JSON during demo setup, preserving the order and speaker of each turn.
...
```python
moments = load_moments(example_path)
assert moments[0].id == "voice-01"
...
- [ ] Implement one nonstreaming call to the configured OpenAI-compatible `/chat/completions` endpoint. Supply all moments in the file plus instructions to treat user decisions as more authoritative than earlier AI proposals. Request a compact JSON packet and validate it before composing text. Reject a context packet longer than 1,600 characters and use the raw utterance instead; never cut the user's request.
...
- [ ] Write `flow-context/README.md` with Python setup, `.env` creation from `.env.example`, a link to the conversation input template, the internal JSON shape for `data/moments.local.json`, and `python -m uvicorn flow_context.server:app --host 127.0.0.1 --port 8000`. Explain that one record represents one attributed turn, that the service rereads the file for each dictation, and that all moments in the file are sent to OpenAI for synthesis. The app itself loads `.env` with `python-dotenv`; commands contain no credential values.
